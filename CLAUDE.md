...
- `lib/widgets/`: widgets shared across screens.
  ...
  - `adaptive.dart`: phone vs iPad/tablet layout (`AppLayout`). Standard phones keep their true size. From `AppLayout.tabletMin` (600) of shortest side a device counts as a tablet. The signed-in shell keeps the floating bottom bar on every device. `AppScale` (passed as `MaterialApp.router(builder:)`) draws the whole UI bigger on large phones and tablets: the app lays out in the window divided by `AppScale.factorFor` and is scaled up to fill it. Plus/Pro Max phones (414pt and wider) get width / 390, so they show the designed 390pt layout drawn bigger; iPad gets 1.2 and iPad Pro 13" 1.3, so text, icons and touch targets grow together and every layout rule sees the scaled size. Content uses the full width (`AppLayout.contentMax` is unlimited; sheets are full width too): the page's list is an `AdaptiveListView` (content clear of the side insets), overview screens (Home, Profile, Stocks, Savings, Card) use `AdaptiveSplit` (summary pane beside detail pane once `AppLayout.splitMin`, 800, of width is available; it applies `EntranceItem.list` itself), and a pinned footer or keypad goes in `AdaptiveCenter`. Shell tabs pad their bottom with `AppLayout.navClearance`. Width decisions use the available width (`LayoutBuilder`), not the window. Phones are locked to portrait (`main()` and `Info.plist`); tablets rotate. Use these for every new screen. `test/adaptive_layout_test.dart` covers the bottom bar, scale and splits, and `test/screens_smoke_test.dart` renders every route on SE, Plus, Pro Max and iPad sizes.
  ...
  - `register_number_field.dart`: `RegisterNumberField`, the input for every editable Mongolian register number (sign-up; the parent link reuses the one saved in `SignUpDraft`). It has two letter boxes that open `showRegisterLetterSheet` (`register_letter_sheet.dart`, the 35-letter alphabet), then an 8-digit field. The screen owns the letters and the digits controller; `RegisterNumberField.validate` gives the shared error messages.
