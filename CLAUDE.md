...
- `appThemeChoice` is global state that outlives a test. Tests that change it reset it to `AppThemeChoice.blue` in `setUp`/`tearDown` (see `test/theme_switch_test.dart`).
